                        ...
                        <div className="flex flex-col space-y-2">
                          ...
                          {request.audit_trail && (
                            <div className="flex items-center space-x-3 text-xs">
                              ...
                              {/* Ztarknet Info */}
                              {(request.audit_trail.l1_tze_tx_id || request.audit_trail.l2_tx_id || request.audit_trail.starknet_tx_hash || request.audit_trail.zcash_tx_id) && (
                                <div className="flex items-center space-x-1">
                          ...
                          {/* Blockchain Details Row - Show actual blockchain info or processing message */}
                          {request.status === 'completed' && (
                            <div className="flex items-center space-x-3 text-xs mt-1">
                              {!request.audit_trail || (!request.audit_trail.starknet_tx_hash && !request.audit_trail.l2_tx_id && !request.audit_trail.l1_tze_tx_id && !request.audit_trail.zcash_tx_id) ? (
                                <div className="text-gray-500 italic">
                                  Processing blockchain anchoring...
                                </div>
                              ) : (
                                <>
                                  {/* L2 Transaction (Starknet) */}
                                  {(request.audit_trail.starknet_tx_hash || request.audit_trail.l2_tx_id) && (
                                    <div className="flex items-center space-x-1">
                                      <span className="text-gray-500">Starknet:</span>
                                      <span className="font-mono text-purple-400" title={request.audit_trail.starknet_tx_hash || request.audit_trail.l2_tx_id || ''}>
                                        {(request.audit_trail.starknet_tx_hash || request.audit_trail.l2_tx_id || '').slice(0, 8)}...
                                      </span>
                                      <a
                                        href={`https://sepolia.starkscan.co/tx/${request.audit_trail.starknet_tx_hash || request.audit_trail.l2_tx_id}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="text-purple-300 hover:text-purple-200 flex items-center"
                                  ...
                                  {/* L1 Transaction (Zcash) */}
                                  {(request.audit_trail.l1_tze_tx_id || request.audit_trail.zcash_tx_id) && (
                                    <div className="flex items-center space-x-1">
                                      <span className="text-gray-500">Zcash:</span>
                                      <span className="font-mono text-green-400" title={request.audit_trail.l1_tze_tx_id || request.audit_trail.zcash_tx_id || ''}>
                                        {(request.audit_trail.l1_tze_tx_id || request.audit_trail.zcash_tx_id || '').slice(0, 8)}...
                                      </span>
                                      <a
                                        href={`https://zcashblockexplorer.com/transactions/${request.audit_trail.l1_tze_tx_id || request.audit_trail.zcash_tx_id}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
